...
generation_node  [END]   ← hallucination guard fires here
   ...
   [END]
    ...
    def build(self):
        ...
        # 2. Add the operational nodes
        graph.add_node("query_rewrite", query_rewrite_node)
        graph.add_node("retrieval", retrieval_node)
        graph.add_node("generation", generation_node)

        # 3. Define the standard directed edges
        graph.add_edge(START, "query_rewrite")
        graph.add_edge("query_rewrite", "retrieval")

        # 4. Add the conditional edge from retrieval to enforce the hallucination guard
        graph.add_conditional_edges(
            "retrieval",
            should_retry_retrieval,
            {
        ...
        # 5. Define the final edge from generation to completion
        graph.add_edge("generation", END)

        # 6. Compile the graph with the memory checkpointer for persistent chat
